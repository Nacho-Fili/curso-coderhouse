...
export default function Cart() {
    ...
    return (
        <div className={styles.itemsContainer} style={{backgroundColor: colors.background}}>
            <div className={styles.itemRow} style={{color: colors.lightFont, borderBottom: `2px solid ${colors.base}`}}>
            ...
            </div>
            {
                items.map(({item, quantity}) => {
                    return(                        
                        ...
                        >
                            <p className={styles.itemTitle} style={{color: colors.lightFont}}>{item.title}</p>
                            <p className={styles.itemQuantity} style={{color: colors.lightFont}}>{quantity} </p>
                            <p className={styles.finalPrice} style={{color: colors.lightFont}}>{`US$${item.price*quantity}`} </p>
                            <p className={"clickable " + styles.itemDelete} style={{color: colors.lightFont}} onClick={() => removeItem(item.id)}> X </p>
                        </div>
                    )
                })
            }
